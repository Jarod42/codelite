...
public:
    ...
    wxOrderedMap(const wxOrderedMap& rhs) = delete;
    ...
    {
        Remove(k);
        auto iter = m_list.emplace(m_list.end(), k, v);
    ...
    {
        Remove(k);
        auto iter = m_list.emplace(m_list.end(), k, std::move(v));
        m_map.emplace(k, iter);
    }
    ...
    {
        Remove(k);
        auto iter = m_list.emplace(m_list.begin(), k, v);
        m_map.emplace(k, iter);
    }
    ...
    {
        Remove(k);
        auto iter = m_list.emplace(m_list.begin(), k, std::move(v));
        m_map.emplace(k, iter);
    }

    const Value& At(const Key& k) const { return m_map.at(k)->second; }
    Value& At(const Key& k) { return m_map.at(k)->second; }
