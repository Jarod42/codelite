...
/** \author Jorgen Bodde
    ...
    This is the wxSerialize documentation guide. wxSerialize is a serializing class meant for serializing /
   de-serializing a complete (or partial) object model to a stream. The streaming is made type safe and reliable. This
   means when an error occurs (wrong data read back or unexpected end of file) the wxSerialize class logs an error, and
   refuses to read from the stream, leaving all variables that follow the error untouched, or in a default state.
    ...
    - Undo / Redo mechanism. Usually undo / redo mechanisms require a lot of overhead in the object model. For example
      when the user deletes a data item in a GUI window, this item should not be really deleted, only taken apart from
   the object model. When using a serializer class, the object can be removed, and later on re-created back on it's
    ...
    -	<b>Reading an old stream in a newer application</b>. So what happens, when for example the older stream does not
   contain the m_monthlyPay and m_healthInsurance, the LeaveObject() boundary was already written in the old code. The
   stream recognises this, and refuses to read data into m_monthlyPay and m_healthInsurance. In these cases the default
    ...
    -	<b>Reading a newer stream with an older application</b>. When the application you released in the field is older
   then the stream being written by the newer release, more data is present in the stream then the old application knows
   about. Here the same situation applies. The LeaveObject() in the old code comes sooner then the expected leave
   boundary in the stream, so wxSerialize will go look for the boundary and ignore all data that is newer.
    ...
    The only drawback that is left with this approach (but is also present in <b>all</b> data writing when older
   applications write back data that contained newer information) is that newer data is being overwritten. You must take
   special caution (for example using the version tag or wxSerializeStatus object) in your application to warn the user
   that the data being written will destroy newer information in the serializing stream. The newer release of your
...
public:
    ...
    */
    void OverwriteNew() { m_newDataLoss = false; };
    ...
    */
    bool NewDataLoss() const { return m_newDataLoss; };
    ...
    void SetNewDataLoss() { m_newDataLoss = true; };
    ...
    const wxString& GetHeader() const { return m_header; };
...
public:
    /** Construct an input stream archive. The stream will be used to retrieve the data from, which can be any
        ...
        version. For example, when version argument is set to 1000, wxSerialize will accept versions 0 .. 1000 for
       reading but will give an error if version 1001 and up is read from the stream. You can use this to set the
       version to at least the supported version for your code. When extending the serialization routines after your app
       has been deployed, the version info can be used to dynamically skip parts while loading from an older stream, and
       warn users that the version of the read stream is older which means incompatibility with the older application
       will occur. This is not the recommended method though, see EnterObject() and LeaveObject() for a more flexible
       solution.
    ...
    */
    wxSerialize(wxInputStream& stream,
                size_t version = 0,
    ...
    */
    wxSerialize(wxOutputStream& stream,
                size_t version = 1,
                const wxString& header = wxEmptyString,
                bool partialMode = false);
    ...
    */
    ...
    const wxString& GetLastErrorStr() const { return m_errMsg; };
    ...
    */
    ...
    /** Indicates current state is writing. This flag will always be true if wxSerialize is constructed with a
        ...
        \endcode
    */
    bool IsStoring() const { return m_writeMode; };
    ...
    /** Indicates current state is reading. This flag will always be true if wxSerialize is constructed with a
        ...
        \endcode
    */
    bool IsLoading() const { return !m_writeMode; };
    ...
    /** This is an extra flag independent of IsStoring() or IsLoading(). It can be set to indicate that certain
        ...
        \endcode
    */
    bool IsFullSerialize() const { return !m_partialMode; };
    ...
    */
    ...
    /** Reads a record from the stream. The wxMemoryBuffer is a growable buffer class, which will contain the data
        ...
        a sequence of bytes to the stream. So be careful storing the whole data back into a structure (little to big
       endian)
    */
    ...
    */
    ...
    /** Saves a double as a wxFloat64 to stream. It writes a header type first, and after that the value. This means
       when the wxFloat64 is read back when it is not expected, an error occurs and the program can terminate gracefully
       because no bogus values are read back. True is returned when the value is saved properly, and false when it's
       not.
    ...
    */
    ...
    /** Saves a wxUint16 to stream. It writes a header type first, and after that the value. This means when the
       wxUint16 is read back when it is not expected, an error occurs and the program can terminate gracefully because
       no bogus values are read back. True is returned when the value is saved properly, and false when it's not.
    ...
    */
    ...
    /** Saves a wxUint32 to stream. It writes a header type first, and after that the value. This means when the
       wxUint32 is read back when it is not expected, an error occurs and the program can terminate gracefully because
       no bogus values are read back. True is returned when the value is saved properly, and false when it's not.
    ...
    */
    ...
    /** Saves a wxUint64 to stream. It writes a header type first, and after that the value. This means when the
       wxUint64 is read back when it is not expected, an error occurs and the program can terminate gracefully because
       no bogus values are read back. True is returned when the value is saved properly, and false when it's not.
    ...
    */
    ...
    /** Saves a wxString to stream. It writes a header type first, and after that the value. This means when the
       wxString is read back when it is not expected, an error occurs and the program can terminate gracefully because
       no bogus values are read back. True is returned when the value is saved properly, and false when it's not.
    ...
    */
    ...
    /** Saves an arbitrary amount of bytes to the stream using wxMemoryBuffer. It writes a header type first, and after
        ...
        Data can be saved up to 4Gb. This is not recommended (nor physically possible I think) but the limit is 32 bits
       for the count.
    ...
    */
    ...
    wxString GetHeaderString() const { return m_headerStr; };
    ...
    */
    ...
    const wxSerializeStatus& GetArchiveStatus() const { return m_status; };
...
private:
    // error types
    enum { wxSERIALIZE_ERR_OK = 0, wxSERIALIZE_ERR_EOF = -1, wxSERIALIZE_ERR_ILL = -2, wxSERIALIZE_ERR_MEM = -3 };
    ...
    int LogError(int err, int msgcode, const wxString& s1 = wxEmptyString, const wxString& s2 = wxEmptyString);
