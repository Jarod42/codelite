    ...
    , m_eventsBound(false)
{
    ...
    EventNotifier::Get()->Bind(wxEVT_BITMAPS_UPDATED, [this](clCommandEvent& event) {
        event.Skip();
        this->SetBitmaps(clBitmaps::Get().GetLoader()->GetStandardMimeBitmapListPtr());
        Refresh();
    });
    BuildTree();
}
...
void FileViewTree::BuildProjectNode(const wxString& projectName)
{
    ...
    if (!prj->IsEmpty()) {
        AppendItem(
            hti, "<dummy>", -1, -1, new FilewViewTreeItemData(ProjectItem("", "", "", ProjectItem::TypeInvalid)));
    }
    ...
    if (projectName == ManagerST::Get()->GetActiveProjectName()) {
        ...
        SetItemFont(hti, f);
        if (!iconFromPlugin) {
            SetItemImage(hti, ACTIVE_PROJECT_IMG_IDX, PROJECT_EXPAND_IMG_IDX);
        }
    }
}
...
{
    if (m_projectsMap.count(project) == 0) {
        return;
    }
...
wxTreeItemId
FileViewTree::FindItemByPath(wxTreeItemId& projectHTI, const wxString& projectPath, const wxString& fileName)
{
    if (!projectHTI.IsOk()) {
        return wxTreeItemId();
    }
    ...
    if (item.IsOk() && clCxxWorkspaceST::Get()->IsOpen()) {
        ...
        if (pi.IsFile()) {
            ...
            if (!projectName.IsEmpty()) {
                ProjectPtr proj = clCxxWorkspaceST::Get()->GetProject(projectName);
                if (proj) {
                    return proj->IsFileExcludedFromConfig(pi.GetFile());
                }
            }
        }
    }
...
void FileViewTree::OnRenameProject(wxCommandEvent& event)
{
    ...
    if (data->GetData().GetKind() == ProjectItem::TypeProject) {
        ...
        if (m_projectsMap.count(newname)) {
            ::wxMessageBox(_("A project with this name already exists in the workspace"),
                           "CodeLite",
...
wxTreeItemId FileViewTree::AddWorkspaceFolder(const wxString& folderPath)
{
    ...
    for (size_t i = 0; i < folders.size(); ++i) {
        ...
        if (m_workspaceFolders.count(current) == 0) {
            ...
            parentItem = AppendItem(
                parentItem, folders.Item(i), WORKSPACE_FOLDER_IMG_IDX, WORKSPACE_FOLDER_EXPANDED_IMG_IDX, cd);
...
void FileViewTree::OnWorkspaceFolderDelete(wxCommandEvent& evt)
{
    ...
    if (::wxMessageBox(
            message, "CodeLite", wxYES_NO | wxCANCEL | wxCANCEL_DEFAULT | wxCENTER, EventNotifier::Get()->TopFrame()) !=
        wxYES) {
        return;
    }
...
void FileViewTree::DoUnbindEvents()
{
    if (m_eventsBound) {
        ...
        frame->Unbind(
            wxEVT_MENU, &FileViewTree::OnAddProjectToWorkspaceFolder, this, XRCID("add_existing_cxx_project"));
        m_eventsBound = false;
    }
}
...
void FileViewTree::DoSetItemBackgroundColour(const wxTreeItemId& item,
...
void FileViewTree::DoAddChildren(const wxTreeItemId& parentItem)
{
    ...
    for (size_t i = 0; i < folders.size(); ++i) {
        ...
        ProjectItem folderItem(
            proj->GetName() + ":" + childVdFullPath, displayName, "", ProjectItem::TypeVirtualDirectory);
...
void FileViewTree::ExcludeFileFromBuildUI(const wxTreeItemId& item, bool exclude)
{
    ...
    if (exclude && m_excludeBuildFiles.count(filename)) {
        return;
    }
    if (!exclude && m_excludeBuildFiles.count(filename) == 0) {
        return;
    }
...
void FileViewTree::OnBuildConfigChanged(wxCommandEvent& e)
{
    ...
    while (!Q.empty()) {
        ...
        if (!ItemHasChildren(item)) {
            ...
            if (d && d->GetData().IsFile()) {
                allFiles.insert({d->GetData().GetFile(), item});
            }
        } else {
            wxTreeItemIdValue k;
...
{
    event.Skip();
    if (event.GetKeyCode() == WXK_NUMPAD_DELETE || event.GetKeyCode() == WXK_DELETE) {
        DoRemoveItems();
    }
}
