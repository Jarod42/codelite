...
// part_w + CTRL+SPACE
TEST_CASE("test_word_completion")
{
...
// test code completion for local variables
TEST_CASE("test_define_with_namespace")
{
...
TEST_CASE("test_trait_alias")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->GetFullName().c_str(), "\\Aliased_Talker");
...
TEST_CASE("test_php7_function_return_value")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->GetFullName().c_str(), "\\test_php7_function_return_value_class");
}
...
TEST_CASE("test_php7_function_arg_hinting")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->GetFullName().c_str(), "\\test_php7_function_arg_hinting_type2");
...
TEST_CASE("test_constants")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->GetFullName().c_str(), "\\MyMoodyClass");
...
TEST_CASE("test_phpdoc_var_in_class" * doctest::may_fail())
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->Type().c_str(), "\\StructB");
...
TEST_CASE("test_phpdoc_property")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->Type().c_str(), "\\ClassWithProps");
...
TEST_CASE("test_phpdoc_method")
{
    ...
    PHPEntityBase::Ptr_t resolved = expr.Resolve(lookup, sourceFile.GetFilename().GetFullPath());
    REQUIRE(resolved);
    CHECK_EQ(resolved->Type().c_str(), "\\ClassWithMethods");
