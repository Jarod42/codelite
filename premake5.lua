...
-- TODO: Use: `%option never-interactive` in file
local function flex_command(prefix)
  prefix = prefix or "%{file.basename}_"
  -- flex -L -B --nounistd --prefix=gdb_result_ -o gdb_result.cpp gdb_result.l
  if #prefix ~= 0 then
...
-- ----------------------------------------------------------------------------
  project "asio"
    ...
    files { "submodules/asio/**.*" }

local function UseAsio()
    includedirs { "submodules/asio/asio/include" }
end
-- ----------------------------------------------------------------------------
  project "cJSON"
    kind "StaticLib"
...
-- ----------------------------------------------------------------------------
  project "websocketpp"
    kind "None" -- header-only
    files { "submodules/websocketpp/**.*" }

local function UseWebsocketpp()
  includedirs { "submodules/websocketpp" }
end
...
-- ----------------------------------------------------------------------------
  project "wxshapeframework"
    kind "SharedLib"
    files { "submodules/wxsf-code/**.*" }
    removefiles { "submodules/wxsf-code/**.rc"}
    includedirs{ "submodules/wxsf-code/include", "submodules/wxsf-code/src" }
    defines {"WXMAKINGDLL_WXXS", "WXMAKINGDLL_WXSF"}

    filter {"files:submodules/wxsf-code/samples/**.*"}
        buildaction "None"
...
local function UseWxShapeFramework()
    includedirs { "submodules/wxsf-code/include" }
    defines { "WXUSINGDLL" }
    links { "wxshapeframework" }
end
...
-- ----------------------------------------------------------------------------
  project "wxflatnotebook" -- to be removed? Not in CMakeList.txt
    kind "None"
    files { "sdk/wxflatnotebook/**.*" }
...
-- ----------------------------------------------------------------------------
  project "libcodelite"
	...
	filter "files:Codelite/Cxx/include_finder.l"
		flex_command("inclf_")
	filter "files:Codelite/XMLLexer.l"
		flex_command("xml")
    filter "toolset:msc*"
	  disablewarnings {
	  "4100", -- '$var': unreferenced formal parameter
...
local function UseLibCodelite()
    UsecJSON()
    UseWxSqlite3()
    includedirs { "Plugin", "Codelite", "CodeLite/ssh", "CxxParser", "PCH", "Interfaces" }
...
-- ----------------------------------------------------------------------------
  project "ALL_FILES"
	...
	files { "art/**.*" }
	files { "cmake/**.*" }
	files { "codelite-icons/**.*" }
	files { "codelite-icons-dark/**.*" }
	files { "codelite-icons-fresh-farm/**.*" }
	files { "codelite-stdio/**.*" }
	files { "docs/**.*" }
	files { "formbuilder/**.*" }
	files { "icons/**.*" }
	files { "InnoSetup/**.*" }
...
-- ----------------------------------------------------------------------------
  project "PHPParser"
	...
	includedirs { "Plugin", "submodules/cJSON", "submodules/wxsqlite3/include", "Codelite", "CodeLite/ssh", "PCH", "Interfaces" }
...
-- ----------------------------------------------------------------------------
  project "SpellChecker"
	kind "SharedLib"
	files { "SpellChecker/**.*" }
	includedirs { "SpellChecker", "SpellChecker/hunspell" }
	-- TODO: Fix hunspell link
	UsePlugin()
	...
	filter { "files:SpellChecker/hunspell/utf_info.cxx" }
...
-- ----------------------------------------------------------------------------
  project "codelite_launcher" -- Not in CMakeList.txt
    kind "ConsoleApp"
	files { "codelite_launcher/**.*" }
...
-- ----------------------------------------------------------------------------
  project "File2Hex" -- Not in CMakeList.txt
    kind "ConsoleApp"
	files { "File2Hex/**.*" }
...
-- ----------------------------------------------------------------------------
  project "FileGrep" -- Not in CMakeList.txt
	kind "ConsoleApp"
	files { "FileGrep/**.*" }
	filter "toolset:msc*"
	  disablewarnings {
...
-- ----------------------------------------------------------------------------
  project "codelite_stdio" -- Not in CMakeList.txt
    kind "ConsoleApp"
    files { "codelite-stdio/**.*" }
    UsePlugin()
...
-- ----------------------------------------------------------------------------
  project "wxcrafter"
	...
	includedirs { "wxcrafter", "wxcrafter/aui", "wxcrafter/controls", "wxcrafter/sizers", "wxcrafter/src", "wxcrafter/top_level_windows", "wxcrafter/wxcLib", "wxcrafter/xrc_handlers" }
	UsePlugin()
