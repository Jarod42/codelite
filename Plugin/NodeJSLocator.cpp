...
void NodeJSLocator::Locate(const wxArrayString& hints)
{
    ...
    if (m_npm.IsEmpty()) {
        m_npm = ::FileUtils::FindExe("npm.cmd", paths).value_or({}).GetFullPath();
    }
#endif

    // Still could not find it, try the PATH environment variable
    if (m_nodejs.empty()) {
        m_nodejs = ::FileUtils::FindExe("node", paths).value_or({}).GetFullPath();
    }

    // No luck? locate npm
    if (m_npm.empty()) {
        m_npm = ::FileUtils::FindExe("npm", paths).value_or({}).GetFullPath();
    }
}
