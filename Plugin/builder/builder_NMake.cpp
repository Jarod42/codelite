...
bool BuilderNMake::Export(const wxString& project,
                          ...
                          wxString& errMsg)
{
    ...
    if (!isProjectOnly) {
        ...
        for (size_t i = 0; i < depsArr.GetCount(); i++) {
            ...
            // is still on the dependency list of this project
            if (!dependProj) {
                wxString msg;
    ...
    if (!isProjectOnly) {
        for (size_t i = 0; i < depsArr.GetCount(); ++i) {
            ...
            if (isPluginGeneratedMakefile) {
                ...
                customWd = ExpandAllVariables(customWd,
                                              clCxxWorkspaceST::Get(),
                                              dependProj->GetName(),
                                              dependProjbldConf->GetName(),
                                              wxEmptyString);
                build_cmd = ExpandAllVariables(build_cmd,
                                               clCxxWorkspaceST::Get(),
                                               dependProj->GetName(),
                                               dependProjbldConf->GetName(),
                                               wxEmptyString);
    ...
    text << "\t@echo \"" << BUILD_PROJECT_PREFIX << project << " - " << projectSelConf << BUILD_PROJECT_SUFFIX
         << "\"\n";
        ...
        for (size_t i = 0; i < depsArr.GetCount(); i++) {
            ...
            if (isPluginGeneratedMakefile) {
                ...
                customWd = ExpandAllVariables(customWd,
                                              ...
                                              dependProjbldConf->GetName(),
                                              wxEmptyString);
                clean_cmd = ExpandAllVariables(clean_cmd,
                                               clCxxWorkspaceST::Get(),
                                               dependProj->GetName(),
                                               dependProjbldConf->GetName(),
                                               wxEmptyString);
...
void BuilderNMake::GenerateMakefile(ProjectPtr proj,
                                    const wxString& confToBuild,
                                    bool force,
...
void BuilderNMake::CreateMakeDirsTarget(ProjectPtr proj,
    ...
    for (size_t i = 0; i < abs_files.size(); i++) {
        ...
        if (cmp->GetCmpFileType(abs_files.at(i).GetExt().Lower(), ft)) {
            ...
            if (ft.kind == Compiler::CmpFileKindSource) {
                ...
                // we construct an object name which also includes the full name of the resource file and appends a .o
                // to the name (to be more precise, $(ObjectSuffix))
                wxString objectName;
                objectName << "$(IntermediateDirectory)\\" << objPrefix << fullnameOnly << "$(ObjectSuffix)";
...
void BuilderNMake::CreateLinkTargets(const wxString& type,
                                     BuildConfigPtr bldConf,
                                     wxString& text,
                                     wxString& targetName,
                                     ...
                                     const wxArrayString& depsProj)
{
    ...
    for (size_t i = 0; i < depsProj.GetCount(); i++) {
        ...
        // Make sure the dependency directory exists
...
wxString
BuilderNMake::GetPOBuildCommand(const wxString& project, const wxString& confToBuild, const wxString& arguments)
{
    wxString errMsg, cmd;
    ProjectPtr proj = clCxxWorkspaceST::Get()->FindProjectByName(project, errMsg);
    if (!proj) {
...
wxString
BuilderNMake::GetPOCleanCommand(const wxString& project, const wxString& confToBuild, const wxString& arguments)
{
    wxString errMsg, cmd;
    ProjectPtr proj = clCxxWorkspaceST::Get()->FindProjectByName(project, errMsg);
    if (!proj) {
...
wxString BuilderNMake::GetSingleFileCmd(const wxString& project,
                                        const wxString& confToBuild,
                                        const wxString& arguments,
...
wxString BuilderNMake::GetPreprocessFileCmd(const wxString& project,
                                            const wxString& confToBuild,
                                            const wxString& arguments,
                                            const wxString& fileName,
                                            wxString& errMsg)
...
wxString BuilderNMake::GetProjectMakeCommand(const wxFileName& wspfile,
...
wxString
BuilderNMake::GetPORebuildCommand(const wxString& project, const wxString& confToBuild, const wxString& arguments)
{
    wxString errMsg, cmd;
    ProjectPtr proj = clCxxWorkspaceST::Get()->FindProjectByName(project, errMsg);
    if (!proj) {
...
wxString BuilderNMake::GetBuildToolCommand(const wxString& project,
                                           const wxString& confToBuild,
                                           const wxString& arguments,
