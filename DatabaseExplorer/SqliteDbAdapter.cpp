...
bool SQLiteDbAdapter::CanConnect() { return m_sFileName != wxT(""); }
...
wxString SQLiteDbAdapter::GetCreateDatabaseSql(const wxString& dbName) { return wxT(""); }
wxString SQLiteDbAdapter::GetDropTableSql(Table* pTab)
{
    // return wxString::Format(wxT("DROP TABLE IF EXISTS '%s'.'%s';"), pTab->GetParentName().c_str(),
    // pTab->GetName().c_str());
    return wxString::Format(wxT("DROP TABLE IF EXISTS '%s';"), pTab->GetName().c_str());
}
wxString SQLiteDbAdapter::GetAlterTableConstraintSql(Table* tab) { return wxT(""); }
wxString SQLiteDbAdapter::GetDropDatabaseSql(Database* pDb) { return wxT(""); }
wxString SQLiteDbAdapter::GetUseDb(const wxString& dbName) { return wxT(""); }
...
IDbAdapter* SQLiteDbAdapter::Clone() { return new SQLiteDbAdapter(m_sFileName); }
